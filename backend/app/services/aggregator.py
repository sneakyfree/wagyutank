...
def _extract_system() -> str:
    ...
    "country (ISO 2-letter — the SELLER's country, not the site's. Directory pages "
    "group breeders under COUNTRY HEADINGS: when a listing sits beneath such a "
    "heading, that heading is its country. Otherwise use an address, a phone country "
    "code, a state/province name or currency. null if the page does not say — never "
    "guess, and never answer 'UN'/'XX' for unknown), "
    "css_status ('css' if the page says CSS/Certified Semen Services or "
